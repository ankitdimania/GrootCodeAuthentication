...
public class AccountActivity extends RoboActionBarActivity implements AccountUtils.AuthenticateCallback,
    ...
    private static final String POST_AUTH_CATEGORY = "com.google.android.iosched.category.POST_AUTH";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            if (!AccountUtils.isAuthenticated(this)) {
                ...
                mChosenAccount = new Account(AccountUtils.getChosenAccountName(this),
                        GoogleAuthUtil.GOOGLE_ACCOUNT_TYPE);

                // Done with set up.
                finishSetup();
                // getSupportFragmentManager().beginTransaction()
                // .add(R.id.root_container, SignInSetupFragment.makeFragment(SETUP_ATTENDEE),
                // "setup_attendee")
                // .commit();
            }
        } else {
    ...
    public void onRecoverableException(final int code) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                final Dialog d = GooglePlayServicesUtil.getErrorDialog(code, AccountActivity.this,
    ...
    public static class SignInMainFragment extends RoboFragment {
            ...
            signinButtonView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    getActivity().getSupportFragmentManager().beginTransaction()
                            .replace(R.id.root_container, new ChooseAccountFragment(), "choose_account")
    ...
    // savedInstanceState) {
    // ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.fragment_login_setup, container,
    // false);
    // final TextView descriptionView = (TextView) rootView.findViewById(R.id.login_setup_desc);
    // descriptionView.setText(Html
    // .fromHtml(getString(mDescriptionHeaderResId) + getString(mDescriptionBodyResId)));
    // return rootView;
    // }
    //
    // @Override
    ...
    // super.onResume();
    // setListAdapter(new ArrayAdapter<String>(getActivity(), R.layout.list_item_login_option,
    // getResources()
    // .getStringArray(mSelectionResId)));
    // }
    //
    // @Override
    ...
    // ((AccountActivity) activity)._finishSetup();
    // } else {
    // getFragmentManager()
    // .beginTransaction()
    // .replace(R.id.root_container, SignInSetupFragment.makeFragment(SETUP_WIFI),
    // "setup_wifi").addToBackStack("setup_attendee").commit();
    // }
    // EasyTracker.getTracker().setCustomDimension(ATCONF_DIMEN_INDEX, "conference attendee");
    //
    // } else if (position == 1) {
    // // Attendee is remote. Set up is done.
    // PrefUtils.setAttendeeAtVenue(activity, false);
    // PrefUtils.setUsingLocalTime(activity, true);
    // EasyTracker.getTracker().setCustomDimension(ATCONF_DIMEN_INDEX, "remote attendee");
    // ((AccountActivity) activity)._finishSetup();
    // }
    ...
    public static class ChooseAccountFragment extends ListFragment {
        ...
        private class AccountListAdapter extends ArrayAdapter<Account> {
            private final int LAYOUT_RESOURCE = R.layout.list_item_login_option;

            public AccountListAdapter(Context context, List<Account> accounts) {
                super(context, R.layout.list_item_login_option, accounts);
            }
    ...
    public static class AuthProgressFragment extends ProgressFragment {
        private static final int TRY_AGAIN_DELAY_MILLIS = 7 * 1000; // 7 seconds

        public AuthProgressFragment() {
            super(TRY_AGAIN_DELAY_MILLIS);
        }

        @Override
        protected boolean onPostDelayed() {
    ...
    public void onAuthTokenAvailable() {
        ...
        if (mAuthProgressFragmentResumed) {
            ...
            // getSupportFragmentManager().popBackStack();
            // getSupportFragmentManager().beginTransaction()
            // .replace(R.id.root_container, SignInSetupFragment.makeFragment(SETUP_ATTENDEE),
            // "setup_attendee")
            // .addToBackStack("signin_main").commit();
        }
    }
